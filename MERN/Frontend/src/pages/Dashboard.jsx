      ...
      <div className="stat-info">
        ...
        {change && (
          <div
            ...
            style={{ color: "#0ca678", background: "rgba(18,184,134,0.08)" }}
          >
            <TrendingUp size={10} />
            <span>{change}</span>
          </div>
...
export default function Dashboard({ user }) {
      ...
      try {
        ...
        opdList
          ...
          .forEach((o) =>
            acts.push({
              id: `opd-${o.id}`,
              text: `${o.patientName} registered for OPD under ${o.doctor}`,
              time: "OPD Registry",
              icon: "👤",
              color: "#4f46e5",
              date: o.dateVisit,
        ...
        ipdList
          ...
          .forEach((i) =>
            acts.push({
              id: `ipd-${i.id}`,
              text: `${i.patientName} admitted to Room ${i.room} / ${i.bed}`,
              time: "IPD Admission",
              icon: "🛏️",
              color: "#0d9488",
              date: i.dateAdmit,
        ...
        apptList
          ...
          .forEach((a) =>
            acts.push({
              id: `appt-${a.id}`,
              text: `Appointment for ${a.patientName} with ${a.doctor}`,
              time: `${a.date} · ${a.time}`,
              icon: "📅",
              color: "#8b5cf6",
              date: a.date,
        ...
        billsList
          ...
          .forEach((b) =>
            acts.push({
              id: `bill-${b.id}`,
              text: `Invoice ${b.invoiceNo} (${b.patientName}) — ${b.status}`,
              time: "Billing",
              icon: "💳",
              color: "#f59e0b",
              date: b.date,
        ...
        setRecentActivities(
          sorted.length > 0
            ...
            : [
                {
                  id: 1,
                  text: "No activities yet. Start by registering a patient!",
                  time: "System",
                  icon: "⚡",
                  color: "#4f46e5",
                },
        ...
        const revData = months
          .map((m) => ({ month: m, revenue: monthlyTotals[m] }))
        ...
        const dData = Object.keys(deptCounts).map((name, i) => ({
          name,
          value: Math.round((deptCounts[name] / total) * 100),
          color: COLORS[i % COLORS.length],
        }));
        setDeptData(
          dData.length > 0
            ? dData
            : [{ name: "General Medicine", value: 100, color: COLORS[0] }],
        );
      } catch (err) {
  ...
  const bedPct =
    s.totalBeds > 0
      ? Math.round(((s.totalBeds - s.availableBeds) / s.totalBeds) * 100)
      : 0;
      ...
      >
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>
            Compiling real-time hospital data
          </p>
        </div>
      </div>
    );
      ...
      <div className="page-body">
        ...
        <div className="grid-stat" style={{ marginBottom: 16 }}>
          ...
          <StatCard
            ...
            value={s.totalIPD}
            color="#0d9488"
            sub={`${s.availableBeds}/${s.totalBeds} beds vacant`}
          />
          <StatCard
            icon={Receipt}
        ...
        >
          ...
          <div className="card" style={{ padding: 20 }}>
            <div className="section-title" style={{ marginBottom: 12 }}>
              <span></span>Monthly Revenue (Paid Invoices)
            </div>
        ...
        >
          ...
          <div className="card" style={{ padding: 20 }}>
            ...
            {[
              ...
              >
                ...
                <span
                  style={{ fontSize: 14.5, fontWeight: 800, color: item.color }}
                >
                  {item.value}
                </span>
              </div>
          ...
          <div className="card" style={{ padding: 20 }}>
            <div className="section-title" style={{ marginBottom: 10 }}>
              <span></span>Recent Clinical Activities
            </div>
