...
export default function AppointmentAdd({ user }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error(err);
        setError(
          "Error fetching patient/doctor lists. Check if backend is running on port 5001.",
        );
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    if (form.doctor && form.date) {
      ...
      fetch(
        `${API_BASE}/appointments?doctorId=${form.doctor}&date=${form.date}`,
      )
        .then((res) => res.json())
        .then((data) => {
  ...
  const getSlotDetails = (slotHour, slotAmPm) => {
    const slotApps = existingAppointments.filter((app) => {
      ...
      const matchesTime =
        appHour === slotHour &&
        ampmVal.toUpperCase() === slotAmPm.toUpperCase();
  ...
  const handleSlotClick = (slotHour, slotAmPm, detail) => {
    if (detail.count >= 1) {
      alert(
        "This time slot is already booked and unavailable. Please select another slot.",
      );
      return;
    }
    setForm((prev) => ({
  ...
  const handleSave = async (e) => {
    ...
    if (detail.count >= 1) {
      return alert(
        `This time slot (${form.hour}:00 ${form.ampm}) is already booked for this doctor. Please choose a different time.`,
      );
    }
  ...
  const selectedDoctorObj = doctors.find(
    (d) => d.id === form.doctor || d._id === form.doctor,
  );
                    ...
                    {patients.map((p) => {
                      ...
                      return (
                        <option key={id} value={id}>
                          {p.patientNo} — {p.firstName || p.firstname}{" "}
                ...
                <div className="form-group">
                  ...
                  >
                    ...
                    {doctors.map((d) => {
                      ...
                      return (
                        <option key={id} value={id}>
                          Dr. {d.firstName || d.firstname}{" "}
                          {d.lastName || d.lastname} ({d.department})
                        </option>
                      );
                    })}
                  </select>
                ...
                <div className="form-group">
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <select
                      ...
                      value={form.ampm}
                      onChange={(e) =>
                        setForm({ ...form, ampm: e.target.value })
                      }
                    >
                      <option>AM</option>
                      <option>PM</option>
            ...
            {!form.doctor ? (
              ...
              >
                ...
                <p style={{ fontSize: 12, marginTop: 4 }}>
                  Select a consultant doctor to inspect availability timeline
                  load graph.
                </p>
              </div>
            ) : (
                ...
                {loadingSchedule ? (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 12 }}>
                      Analyzing doctor's agenda loads...
                    </span>
                  </div>
                ) : (
                  <div
                  ...
                  >
                    ...
                    >
                      Click on a vertical slot bar below to instantly pick that
                      appointment hour.
                    </p>
                    ...
                    >
                      ...
                      >
                        ...
                        {TIMELINE_SLOTS.map((slot) => {
                          ...
                          const isSelected =
                            form.hour === slot.hour && form.ampm === slot.ampm;
                          ...
                          return (
                            <div
                              key={slot.label}
                              onClick={() =>
                                handleSlotClick(slot.hour, slot.ampm, detail)
                              }
                              style={{
                                display: "flex",
                                flexDirection: "column",
                                ...
                                flex: 1,
                                cursor:
                                  detail.count >= 2 ? "not-allowed" : "pointer",
                                height: "100%",
