...
const initialBills = [
  ["SI-000027", "P-000001", "2024-05-10 10:00:00", 1080.0, "Paid", "Cash"],
  [
    "SI-000028",
    "P-000002",
    "2024-05-12 10:00:00",
  ...
  ["SI-000029", "P-000003", "2024-05-15 10:00:00", 3000.0, "Partial", "Cash"],
  [
    "SI-000030",
    "P-000004",
    "2024-05-08 10:00:00",
...
const connectDB = async () => {
  ...
  const dbName = process.env.DB_NAME || "hmsai";
  ...
  if (!tempConn) {
    console.log(
      "⚠️  MySQL Connection Error: Access denied for all probed credentials.",
    );
    console.log(
      "📡 [Dual-Mode DB] MySQL not available. Running seamlessly in local JSON-file Database Mode.",
    );
    dbType = "json";
    return;
  }
  ...
  try {
    ...
    try {
      ...
      try {
        const [roomCols] = await connection.query(
          "SHOW COLUMNS FROM `room_master` LIKE 'id'",
        );
        if (roomCols.length > 0) {
          console.log(
            "🔄 Old room_master table schema detected. Dropping for schema migration...",
          );
      ...
      try {
        const [opdCols] = await connection.query(
          "SHOW COLUMNS FROM `opd_records` LIKE 'id'",
        );
        if (opdCols.length > 0) {
          console.log(
            "🔄 Old opd_records table schema detected. Dropping for schema migration...",
          );
      ...
      try {
        await connection.query(
          "ALTER TABLE `nurse_discharge` ADD COLUMN `summary_data` LONGTEXT DEFAULT NULL",
        );
      } catch (err) {
      ...
      const [usrs] = await connection.query(
      ...
      const [pts] = await connection.query(
        "SELECT COUNT(*) as count FROM patient_personal_info",
      );
      if (pts[0].count === 0) {
        await connection.query(
      ...
      const [apps] = await connection.query(
        "SELECT COUNT(*) as count FROM patient_appointment",
      );
      if (apps[0].count === 0) {
        await connection.query(
      ...
      const [rms] = await connection.query(
        "SELECT COUNT(*) as count FROM room_master",
      );
      if (rms[0].count === 0) {
        await connection.query(
      ...
      const [opds] = await connection.query(
        "SELECT COUNT(*) as count FROM opd_records",
      );
      if (opds[0].count === 0) {
        await connection.query(
      ...
      const [ipds] = await connection.query(
        "SELECT COUNT(*) as count FROM ipd_admissions",
      );
      if (ipds[0].count === 0) {
        await connection.query(
      ...
      const [ambs] = await connection.query(
        "SELECT COUNT(*) as count FROM ambulance_fleet",
      );
      if (ambs[0].count === 0) {
        await connection.query(
      ...
      const [bCheck] = await connection.query(
        "SELECT COUNT(*) as count FROM building_master",
      );
      if (bCheck[0].count === 0) {
        await connection.query(`
      ...
      // users migrations
      await safeAlter(
        `ALTER TABLE users ADD COLUMN date_entry datetime NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER InActive`,
      );
      await safeAlter(
        `ALTER TABLE users MODIFY COLUMN title int(11) NOT NULL DEFAULT 0`,
      );

      // room_master migrations
      await safeAlter(
        `ALTER TABLE room_master ADD COLUMN floor int(11) NOT NULL DEFAULT 1 AFTER price_per_day`,
      );
      await safeAlter(
        `ALTER TABLE room_master ADD COLUMN building varchar(100) NOT NULL DEFAULT 'Main Building' AFTER floor`,
      );
      await safeAlter(
        `ALTER TABLE room_master ADD COLUMN total_floors int(11) NOT NULL DEFAULT 5 AFTER building`,
      );
      try {
        await connection.query(`
      ...
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN registration_no   varchar(50)  DEFAULT ''        AFTER plate`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN driver_user_id    varchar(20)  DEFAULT NULL      AFTER driver`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN insurance_expiry  date         DEFAULT NULL      AFTER phone`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN fitness_expiry    date         DEFAULT NULL      AFTER insurance_expiry`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN permit_expiry     date         DEFAULT NULL      AFTER fitness_expiry`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN gps_device_id     varchar(50)  DEFAULT NULL      AFTER permit_expiry`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN current_lat       decimal(10,7) DEFAULT NULL     AFTER gps_device_id`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN current_lng       decimal(10,7) DEFAULT NULL     AFTER current_lat`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN odometer_reading  int(11)      DEFAULT 0         AFTER current_lng`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN hospital_id       varchar(20)  DEFAULT 'HOSP-01' AFTER odometer_reading`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN is_active         tinyint(1)   NOT NULL DEFAULT 1 AFTER hospital_id`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN created_by        varchar(50)  DEFAULT 'system'  AFTER is_active`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN updated_by        varchar(50)  DEFAULT NULL      AFTER created_by`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN created_at        datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER updated_by`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet ADD COLUMN updated_at        datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP AFTER created_at`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_fleet MODIFY COLUMN status enum('Available','Dispatched','Maintenance','Out of Service') DEFAULT 'Available'`,
      );

      // ambulance_dispatch migrations
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN patient_no          varchar(30)   DEFAULT NULL  AFTER patient_name`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN caller_name         varchar(100)  DEFAULT ''    AFTER patient_no`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN caller_relation     varchar(50)   DEFAULT ''    AFTER caller_name`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN pickup_lat          decimal(10,7) DEFAULT NULL  AFTER pickup_location`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN pickup_lng          decimal(10,7) DEFAULT NULL  AFTER pickup_lat`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN assigned_driver_id  varchar(20)   DEFAULT NULL  AFTER vehicle_id`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN assigned_paramedic_id varchar(20) DEFAULT NULL  AFTER assigned_driver_id`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN call_received_at    datetime      DEFAULT CURRENT_TIMESTAMP AFTER severity`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN pickup_time         datetime      DEFAULT NULL  AFTER dispatch_date`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN hospital_arrival_time datetime     DEFAULT NULL  AFTER pickup_time`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN closed_at           datetime      DEFAULT NULL  AFTER hospital_arrival_time`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN eta_minutes         int(11)       DEFAULT NULL  AFTER closed_at`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN odometer_start      int(11)       DEFAULT NULL  AFTER eta_minutes`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN odometer_end        int(11)       DEFAULT NULL  AFTER odometer_start`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN trip_distance_km    decimal(8,2)  DEFAULT NULL  AFTER odometer_end`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN bill_id             varchar(30)   DEFAULT NULL  AFTER trip_distance_km`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN opd_id              varchar(30)   DEFAULT NULL  AFTER bill_id`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN ipd_id              varchar(30)   DEFAULT NULL  AFTER opd_id`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN call_received_by    varchar(50)   DEFAULT NULL  AFTER ipd_id`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN dispatched_by       varchar(50)   DEFAULT NULL  AFTER call_received_by`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN closed_by           varchar(50)   DEFAULT NULL  AFTER dispatched_by`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN closure_notes       text          DEFAULT NULL  AFTER closed_by`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN hospital_id         varchar(20)   DEFAULT 'HOSP-01' AFTER closure_notes`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN created_at          datetime      NOT NULL DEFAULT CURRENT_TIMESTAMP`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch ADD COLUMN updated_at          datetime      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch MODIFY COLUMN dispatch_id varchar(30) NOT NULL`,
      );
      await safeAlter(
        `ALTER TABLE ambulance_dispatch MODIFY COLUMN status enum('Active','Picked Up','Arrived','Completed','Cancelled') DEFAULT 'Active'`,
      );

      console.log(
        "🎉 MySQL Database fully created and seeded with all HMS clinical & administrative data!",
      );
    } finally {
      connection.release();
    }
    ...
    console.log(
      "📡 [Dual-Mode DB] Reverting to local JSON-file Database Mode.",
    );
    dbType = "json";
  }
};
